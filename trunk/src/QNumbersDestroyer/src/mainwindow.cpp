...
{
	qsetAppSettings=new QSettings(QSettings::IniFormat,QSettings::UserScope,"PanteR","QNumbersDestroyer");
...
{
	qwMainWidget=new QWidget(this);
	this->setCentralWidget(qwMainWidget);
//
...
//
	qdwInformationPanel=new QDockWidget(tr("Information panel"));
	qdwInformationPanel->setObjectName("Information panel");
	qdwInformationPanel->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
	this->addDockWidget(Qt::RightDockWidgetArea,qdwInformationPanel);
	qmipInformationPanel=new QMyInformationPanel(qdwInformationPanel);
	qdwInformationPanel->setWidget(qmipInformationPanel);
//
...
void MainWindow::setLayouts()
{
	...
	qvblMainLayout->addWidget(qtvTable);

	qwMainWidget->setLayout(qvblMainLayout);
}
//
...
{
	this->restoreState(qsetAppSettings->value("MainWindow/state",QByteArray()).toByteArray());
...
{
	qsetAppSettings->setValue("MainWindow/state",this->saveState());
...
//
void MainWindow::createGame(int columnsCount, int numbersCount) //Создание игры
{
	qsimModel->clear();
	nullInformation();
...
//
void MainWindow::checkndel(QStandardItem* current, QStandardItem* previous)
{
	qsiPreviousItem=0;
	qtvTable->clearFocus();
...
//
void MainWindow::slotCellClicked (const QModelIndex& qmiIndex)
{
...
//
void MainWindow::slotUpdateGame()
{
	if (qsimModel->rowCount()==0)
		return;
	setLastItem();
...
//
bool MainWindow::isRowEmpty(int iRow)
{
...
void MainWindow::createToolBars()
{
	...
	this->addToolBar(qtbGameToolbar);
}
//
...
//
bool MainWindow::openGame(const QString& qsFileName)
{
	nullInformation();
...
//
//
bool MainWindow::slotSaveAsGame()
{
	if (qsimModel->rowCount()==0)
		return false;
	QString qsFileName;
...
//
bool MainWindow::saveGame(const QString& qsFileName)
{
	qsdBase.setDatabaseName(qsFileName);
	if (!qsdBase.open())
	{
...
//
void MainWindow::slotUpdateInformation()
{
...
//
QList<QStandardItem*> MainWindow::itemsForDelete(const QStandardItem& item)
{
...
//
void MainWindow::deleteItems(QStandardItem* current, QStandardItem* previous)
{
...
//
void MainWindow::nullInformation()
{
	iMSecOffset=0;
	iKillItemsCount=0;
	iHelpCount=0;
...
//
void MainWindow::blinkItem(QStandardItem* current, const int iWaitTime, const int iBlinkCount)
{
